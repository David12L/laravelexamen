    ...
    <form action="{{ route('Estudiante.xUpdate', $xActAlumnos->id) }}" method="post" class="d-grid gap-2">
        ...
        @if($errors ->has('apeEst'))
            <div class="alert alert-warning alert-dismissible fade show" role="alert">
                El <strong>curso </strong> es requerido
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
        @endif

        <input type="text" name="codEst" placeholder="Código" value="{{ $xActAlumnos->codEst }}" class="form-control mb-2">
        <input type="text" name="nomEst" placeholder="curso" value="{{ $xActAlumnos->nomEst }}" class="form-control mb-2">
        <input type="text" name="apeEst" placeholder="horas" value="{{ $xActAlumnos->apeEst }}" class="form-control mb-2">
        <input type="date" name="fnaEst" placeholder="Fecha de inicio de curso" value="{{ $xActAlumnos->fnaEst }}" class="form-control mb-2">
        ...
        <select name="modMat" class="form-control mb-2">
            <option value="">Seleccione modalidad del curso...</option>
            <option value="S" @if ($xActAlumnos->modMat == "S") {{ "SELECTED" }} @endif>transeversal</option>
            <option value="R" @if ($xActAlumnos->modMat == "R") {{ "SELECTED" }} @endif>especialidad</option>
        </select>
